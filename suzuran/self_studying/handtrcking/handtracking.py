                ...
                if finger_points:
                    ...
                    if text == 'no!!!':
                        ...
                        if f3 < 9 and f3 > 0:            # 如果中指角度小於 50
                            ...
                            mosaic = img[y_min:y_max-120, x_min+20:x_max-20]     # 取出四邊形區域
                            mosaic = cv2.resize(mosaic, (5,5), interpolation=cv2.INTER_LINEAR)  # 根據縮小尺寸縮小
                            mosaic = cv2.resize(mosaic, (mosaic_w-40,mosaic_h-120), interpolation=cv2.INTER_NEAREST) # 放大到原本的大小
                            img[y_min:y_max-120, x_min+20:x_max-20] = mosaic    # 馬賽克區域
                        else:
                            x_max = max(fx)              # 如果是比中指，取出 x 座標最大值
                            y_max = max(fy)              # 如果是比中指，取出 y 座標最大值
                            x_min = min(fx) - 10         # 如果是比中指，取出 x 座標最小值
